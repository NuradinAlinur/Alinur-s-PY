...
class DatabaseManager:
    def __init__(self):
        self.conn = psycopg2.connect(
            dbname="new_database",  # Убедись, что эта база данных существует
            user="postgres",
            password="20070531",
            host="localhost",
            port="5432"
...
class UserManager:
    ...
    def get_user_id(self):
        ...
        self.db.cur.execute(query, (self.username,))
        # Исправлено: теперь правильно обрабатываем результат
        self.user_id = self.db.cur.fetchone()[0]
        print(self.user_id)
    ...
    def load_user(self, users):
        ...
        if not data:  # Если данных нет, создаём нового пользователя
            self.create_user()
            self.level = 1
            self.score = 0
        else:
            data = data[-1]  # Берём последние данные пользователя
            self.level = data[2]
            self.score = data[3]
...
class Snake:
    ...
    def draw(self, surface):
        for segment in self.body:
            pygame.draw.rect(
                surface, GREEN, (*segment, self.cell_size, self.cell_size))
...
class LevelManager:
    ...
    def draw(self, surface):
        text = font_small.render(
            f"Score: {self.score}  Level: {self.level}", True, WHITE)
...
while running:
    ...
    if not paused:
        ...
        if snake.collided():
            ...
            score_text = font_middle.render(
                f"Your score: {level.score}", True, BLACK)
            screen.blit(score_text, (170, 220))
            level_text = font_middle.render(
                f"Your level: {level.level}", True, BLACK)
